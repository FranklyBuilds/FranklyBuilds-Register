...
- 当前 focused Outlook/Mongo API 测试：46 passed；全后端测试：782 passed、14 skipped。
...
- 后端：本次全量 `app/tests/backend` 为 782 passed、14 skipped；Python compileall 和 git diff --check 通过。MailCom 兼容相关测试包含在该结果中。
...
- 修复 Outlook 任务 reset 后遗留 `proxyGroup`/`proxyCount` 的生命周期元数据；重点测试现为 46 passed，重启主服务后通过 HTTP 验证 reset 会清空旧代理元数据。
