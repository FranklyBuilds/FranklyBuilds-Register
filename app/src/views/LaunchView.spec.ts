...
describe('LaunchView', () => {
  it('defaults to browser registration and exposes both registration modes', () => {
    ...
    expect(mode.props('modelValue')).toBe('browser')
    expect(mode.props('options')).toEqual([
  ...
  it('launches the protocol endpoint path when protocol registration is selected', async () => {
    const pinia = createPinia()
    setActivePinia(pinia)
    const store = useAppStore()
    store.stats.emails.available = 2
    store.mongoHealth.status = 'online'
    store.settings.concurrency = 1
    const protocolStart = vi.spyOn(store, 'startProtocolRegistrationRun').mockResolvedValue({
      ...store.runState,
      status: 'completed',
      runId: 'run-protocol',
    ...
    const browserStart = vi.spyOn(store, 'startBrowserProbeRun')

    const wrapper = mount(LaunchView, {
      global: { plugins: [pinia, ElementPlus] },
    })
    wrapper.getComponent({ name: 'ElSegmented' }).vm.$emit('update:modelValue', 'protocol')
    const countrySelect = wrapper.findAllComponents({ name: 'ElSelect' })[1]
    countrySelect!.vm.$emit('update:modelValue', 'TR')
    await nextTick()
    await wrapper.get('.start-button').trigger('click')
    await flushPromises()

    expect(protocolStart).toHaveBeenCalledWith(1, 'TR', '', 'all')
    expect(browserStart).not.toHaveBeenCalled()
    expect(wrapper.text()).toContain('需要 workspace 0')
  })
