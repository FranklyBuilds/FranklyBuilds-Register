...
type RegistrationMode = 'browser' | 'protocol'

const store = useAppStore()
// Kept as an explicit compatibility choice; an empty selection uses Mongo's
// directly imported default proxy group instead.
const LOCAL_PROXY_GROUP = '__local_127_0_0_1_7890__'
const registrationMode = ref<RegistrationMode>('browser')
const registrationModeOptions = [
  { label: '浏览器注册', value: 'browser' },
  { label: '协议注册', value: 'protocol' },
]
...
const registrationModeLabel = computed(() =>
  registrationMode.value === 'protocol' ? '协议注册' : '浏览器注册',
)
const modeDescription = computed(() =>
  registrationMode.value === 'protocol'
    ? '使用独立协议会话完成注册，成功账号自动进入账号池。'
    : '使用 Roxy/Playwright 完成浏览器注册，成功账号自动进入账号池。',
)
...
const workspaceRequired = computed(() =>
  registrationMode.value === 'browser' && Math.max(0, Number(requestedCount.value) || 0) > 0 ? 1 : 0,
)
...
watch(
  ...
  (kind) => {
    if (!running.value) return
    registrationMode.value = kind === 'protocol_registration' ? 'protocol' : 'browser'
  },
  { immediate: true },
)
...
watch(
  ...
  (options) => {
    if (
      selectedGroup.value &&
      selectedGroup.value !== LOCAL_PROXY_GROUP &&
      !options.some((item) => item.group === selectedGroup.value)
    ) {
      selectedGroup.value = ''
    }
  },
...
function workerStageLabel(stage: string) {
  const labels: Record<string, string> = {
    ...
    auth_flow: '协议认证',
    user_register: '创建用户',
    email_otp_send: '发送验证码',
    email_otp_wait: '等待验证码',
    email_otp_validate: '校验验证码',
    create_account: '创建账号',
    auth_session: '读取会话',
    existing_login: '已有账号登录',
    proxy_check: '检查代理',
    login: '登录页',
    email: '提交邮箱',
    verification: '验证码',
...
function selectedProxyLabel() {
  if (selectedGroup.value === LOCAL_PROXY_GROUP) return '本机代理 127.0.0.1:7890'
  return selectedGroup.value || '默认组（直接导入）'
}
...
async function startRun() {
  ...
  try {
    const start = registrationMode.value === 'protocol'
      ? store.startProtocolRegistrationRun
      : store.startBrowserProbeRun
    ...
    if (result.status === 'completed') {
      ElMessage.success(`${registrationModeLabel.value}完成：成功 ${result.succeeded}，失败 ${result.failed}`)
    } else {
      ElMessage.warning(`${registrationModeLabel.value}结束：${runStatusLabel()}`)
    }
  } catch (error) {
    ElMessage.error(error instanceof Error ? error.message : `${registrationModeLabel.value}启动失败`)
  }
}
...
<template>
  <section>
    <div class="page-heading launch-heading">
      <div>
        <h2>任务启动台</h2>
        <p>FastAPI 使用多进程调度浏览器或协议注册，每个邮箱使用独立 worker。</p>
      </div>
    ...
    <div class="launch-workspace">
      <article class="panel start-panel">
        <div class="section-heading">
          ...
          <div><h3>启动注册任务</h3><p>{{ modeDescription }}</p></div>
        </div>
        ...
        <div class="run-form">
          ...
          <el-select
            ...
            clearable
            filterable
            placeholder="不选择则使用默认组（直接导入）"
            :disabled="running || !selectedCountry"
          >
            <el-option label="兼容本机代理 · 127.0.0.1:7890" :value="LOCAL_PROXY_GROUP" />
            <el-option
          ...
          <div class="run-hints">
            ...
            <span>代理 {{ selectedProxyLabel() }}</span>
          ...
          <div class="run-actions">
            <el-button
              ...
              @click="startRun"
            >
              {{ store.runState.status === 'waiting_for_database' ? '等待数据库恢复' : running ? `${registrationModeLabel}运行中` : `启动${registrationModeLabel}` }}
            </el-button>
            <el-button
              v-if="running"
              class="cancel-button"
      ...
      <article class="panel progress-panel">
        ...
        />
        <el-alert
          v-if="store.runState.kind === 'browser_probe' && store.runState.terminalReasonCode === 'roxy_circuit_open'"
          class="circuit-alert"
          type="error"
          :closable="false"
        ...
        <div class="rule-note">
          ...
          <span v-if="registrationMode === 'protocol'">每个 worker 使用独立协议会话、代理和邮箱资源。</span>
          <span v-else>所有 worker 共享一个 workspace；每个 worker 使用独立进程、代理和临时指纹窗口。</span>
        </div>
      </article>
    </div>
    ...
    <article class="panel log-panel">
      ...
      <div ref="logViewport" class="log-viewport">
        <div v-if="!store.displayedLogs.length" class="empty-log">
          ...
          <span>暂无任务日志，启动一次注册任务后将在这里实时显示。</span>
        </div>
        <div
...
.section-icon {
  ...
  flex: 0 0 38px;
  place-items: center;
  border: 1px solid rgb(213 175 0 / 30%);
  border-radius: 10px;
  color: var(--accent);
...
.registration-mode :deep(.el-segmented__item) {
  min-width: 0;
  flex: 1;
}
...
.log-row:hover { background: rgb(213 175 0 / 9%); }
