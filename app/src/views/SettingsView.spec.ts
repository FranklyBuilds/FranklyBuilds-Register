...
describe('SettingsView proxy pool deletion', () => {
  it('exposes direct HTTP/SOCKS import without requiring an external manager', async () => {
    const pinia = createPinia()
    setActivePinia(pinia)
    const store = useAppStore()
    vi.spyOn(store, 'refreshProxies').mockResolvedValue(proxyPage([], 0, store.proxyQuery))
    vi.spyOn(store, 'refreshProxyCountries').mockResolvedValue([])
    vi.spyOn(store, 'refreshProxyGroups').mockResolvedValue([])
    ...
    expect(wrapper.text()).toContain('HTTP、HTTPS、SOCKS5 / SOCKS5H')
    expect(wrapper.text()).toContain('TXT / YAML')
    expect(wrapper.text()).not.toContain('Easy Proxies')
    expect(wrapper.text()).not.toContain('Resin')

    await buttonByText(wrapper, '导入 HTTP / SOCKS').trigger('click')
    await nextTick()
    expect(wrapper.getComponent({ name: 'ImportDialog' }).props('modelValue')).toBe(true)
  })
