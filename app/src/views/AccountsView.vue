...
import { CircleCheck, CopyDocument, CreditCard, Delete, Download, Key, Link, Lock, MagicStick, Refresh, Search, UserFilled } from '@element-plus/icons-vue'
import { ElMessage, ElMessageBox, type TableInstance } from 'element-plus'
import type { AccountRecord, EmailChangeRun, ExportFormat, ExportScope, ProxyRecord, ResourceQuery } from '@/types'
...
const emailChangeAccount = ref<AccountRecord | null>(null)
const emailChangeActiveAccountId = ref<string | null>(null)
let emailChangeCompletionGeneration = 0
...
const emailChangeDeleteLocked = computed(() => Boolean(
  emailChangeActiveAccountId.value && selectedIds.value.includes(emailChangeActiveAccountId.value),
))
...
function openEmailChange(account: AccountRecord) {
  if (emailChangeActiveAccountId.value) {
    ElMessage.warning('已有邮箱换绑任务进行中，请先完成或取消它')
    return
  }
  emailChangeCompletionGeneration += 1
  emailChangeAccount.value = account
  emailChangeOpen.value = true
}

function updateEmailChangeActivity(accountId: string | null) {
  emailChangeActiveAccountId.value = accountId
}

async function handleEmailChangeCompleted(run: EmailChangeRun) {
  const accountId = emailChangeAccount.value?.id
  if (!accountId || (run.accountId && run.accountId !== accountId)) return
  const completionGeneration = ++emailChangeCompletionGeneration

  // Close the completed dialog before awaiting list refreshes.  The refreshes
  // may outlive this dialog and a new account can be opened in the meantime;
  // the generation guard below prevents this handler from clearing that new
  // context when its finally block runs.
  emailChangeOpen.value = false
  emailChangeAccount.value = null
  emailChangeActiveAccountId.value = null
  try {
    await Promise.all([
      loadPage(),
      store.refreshEmails(),
    ...
    ElMessage.success('账号邮箱已更新')
  } catch (error) {
    ElMessage.warning(error instanceof Error ? error.message : '账号已换绑，但列表刷新失败')
  } finally {
    if (completionGeneration !== emailChangeCompletionGeneration) return
    emailChangeOpen.value = false
    emailChangeAccount.value = null
    emailChangeActiveAccountId.value = null
  }
...
async function deleteSelected() {
  ...
  if (emailChangeDeleteLocked.value) {
    ElMessage.warning('邮箱换绑任务进行中，暂不能删除该账号')
    return
  }
  ...
  // The task can become active while the confirmation dialog is open.
  if (emailChangeDeleteLocked.value) {
    ElMessage.warning('邮箱换绑任务进行中，暂不能删除该账号')
    return
  }
...
<template>
  <section>
    ...
    <div class="panel table-panel">
      <div class="table-toolbar">
        ...
        <div class="toolbar-group">
          ...
          <el-button
            ...
            plain
            :icon="Delete"
            :disabled="selectedIds.length === 0 || emailChangeDeleteLocked"
      ...
      >
        ...
        <el-table-column
          ...
          width="216"
          fixed="right"
          class-name="account-actions-column"
          label-class-name="account-actions-column"
        >
          <template #default="{ row }">
            <div class="account-row-actions">
              <el-tooltip content="检测账号是否仍然有效" effect="light">
                <span>
                  <el-button
                    text
                    type="success"
              ...
              <el-tooltip content="查询 Plus 试用资格" effect="light">
                <span>
                  <el-button
                    text
                    type="warning"
              ...
              <el-tooltip content="单独检测账单类型" effect="light">
                <span>
                  <el-button
                    text
                    type="primary"
              ...
              <el-tooltip content="使用代理池全部可用代理检测 OAICS，并按国家统计成功率" effect="light">
                <span>
                  <el-button
                    text
                    type="warning"
              ...
              <el-tooltip content="更换账号邮箱" effect="light">
                <span>
                  <el-button
                    text
                    type="warning"
                    :icon="Refresh"
                    aria-label="更换邮箱"
                    :loading="emailChangeActiveAccountId === row.id"
                    :disabled="emailChangeActiveAccountId !== null"
                    @click="openEmailChange(row)"
                  />
                </span>
              </el-tooltip>
              <el-tooltip content="打开接码 URL" effect="light">
                <span>
                  <el-button
                    text
                    type="primary"
              ...
              <el-tooltip content="导出账号" effect="light">
                <span>
                  <el-button
                    text
                    type="primary"
                    :icon="Download"
                    aria-label="导出账号"
                    @click="openExport('single', row)"
                  />
                </span>
              </el-tooltip>
              <el-tooltip
                :content="hasValidAccessToken(row) ? '复制或下载已保存的 AT' : '该账号没有有效 AT'"
                effect="light"
              >
                <span>
                  <el-button
    ...
    <EmailChangeDialog
      v-model="emailChangeOpen"
      :account="emailChangeAccount"
      @active-change="updateEmailChangeActivity"
      @completed="handleEmailChangeCompleted"
    />
  </section>
</template>
...
.account-row-actions :deep(.el-tooltip__trigger) {
  display: flex;
  width: 34px;
  height: 34px;
}

.account-row-actions :deep(.el-button) {
  width: 34px;
  height: 34px;
  margin: 0;
  padding: 0;
...
:deep(.account-actions-column) {
  border-left: 1px solid var(--border-subtle);
  background: var(--bg-surface) !important;
}

:deep(.el-table__body tr:hover > td.account-actions-column) {
  background:
    linear-gradient(var(--el-table-row-hover-bg-color), var(--el-table-row-hover-bg-color)),
...
.email-avatar {
  ...
  flex: 0 0 31px;
  place-items: center;
  border: 1px solid rgb(213 175 0 / 28%);
  border-radius: 9px;
  color: var(--accent-strong);
  background: rgb(213 175 0 / 9%);
  font-size: 11px;
  font-weight: 800;
}
