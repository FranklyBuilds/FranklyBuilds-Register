...
<template>
  <section class="mailcom-view">
    <el-card shadow="never">
      ...
      <el-alert v-if="migration" class="migration-result" :type="migration.errors ? 'warning' : 'success'" :closable="false" :title="migration.status === 'missing' ? '没有发现旧 MailCom SQLite 数据库' : `SQLite 迁移：账号 ${migration.accounts ?? 0}、别名 ${migration.aliases ?? 0}、新增 ${migration.imported ?? 0}、重复 ${migration.duplicates ?? 0}、错误 ${migration.errors ?? 0}`" />
