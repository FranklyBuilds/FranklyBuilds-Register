...
async function pollActiveTasks() {
  const activeTasks = tasks.value.filter((task) => !TERMINAL_STATES.has(task.status))
  if (!activeTasks.length) {
    stopPolling()
    return
  }

  // Refresh active rows independently so a completed task updates immediately
  // without replacing terminal rows that the user may still be reviewing.
  let refreshFailed = false
  const refreshed = await Promise.all(activeTasks.map(async (task) => {
    try {
      return await dataGateway.getPaymentExtractorTask(task.taskId)
    } catch {
      refreshFailed = true
      return null
    }
...
<style scoped>
.stored-source-box { padding: 12px; margin-bottom: 4px; border: 1px solid var(--border-color); border-radius: 10px; background: #f7f8f5; }
...
.token-item.active {
  border-color: rgb(213 175 0 / 55%);
  background: rgb(213 175 0 / 10%);
}
...
.task-card { padding: 14px; border: 1px solid var(--border-subtle); border-radius: 12px; background: var(--surface-soft); }
...
.task-logs { display: grid; gap: 3px; max-height: 120px; padding: 9px; margin-top: 10px; overflow: auto; border-radius: 8px; background: var(--code-bg); }
...
.result-url code { min-width: 0; color: #236c4a; font-size: 12px; overflow-wrap: anywhere; }
