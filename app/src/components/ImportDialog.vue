...
const formatHint = computed(() =>
  isEmail.value
    ? '邮箱----接码地址 或 邮箱----mail.com 密码'
    : 'http://主机:端口、socks5://主机:端口 或 host:port[:username:password]',
)
...
const yamlProxyInput = computed(() =>
  !isEmail.value && /^\s*proxies\s*:/m.test(rawText.value),
)
const yamlProxyCount = computed(() => {
  if (!yamlProxyInput.value) return 0
...
async function submit() {
  if ((!preview.value.accepted.length && !yamlProxyInput.value) || submitting.value) return
  ...
  >
    <div class="import-intro">
      ...
      <div>
        ...
        <p>代理支持无认证或带认证的 HTTP / SOCKS 地址、proxies: 列表 YAML；可粘贴或选择 UTF-8 TXT/YAML 文件。</p>
      </div>
    </div>
    ...
    <div v-if="yamlProxyInput" class="import-preview import-preview--yaml">
      <div class="preview-count preview-count--ok">
        <strong>{{ yamlProxyCount || '—' }}</strong>
        <span>{{ yamlProxyCount ? 'YAML 节点（后端复核）' : 'YAML（后端解析）' }}</span>
      </div>
    </div>
    ...
    <div v-if="!yamlProxyInput && (preview.duplicates.length || preview.errors.length)" class="issue-list">
    ...
    <template #footer>
      ...
      <el-button
        type="primary"
        :disabled="preview.accepted.length === 0 && !yamlProxyInput"
        :loading="submitting"
        @click="submit"
      >
        确认导入 {{ yamlProxyInput ? (yamlProxyCount ? `${yamlProxyCount} 条 YAML` : 'YAML') : `${preview.accepted.length} 条` }}
      </el-button>
    </template>
  </el-dialog>
</template>
