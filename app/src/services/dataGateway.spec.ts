...
describe('dataGateway direct proxy import', () => {
  beforeEach(() => {
    vi.stubGlobal('fetch', vi.fn())
  })

  it('sends raw HTTP/SOCKS text to the direct import endpoint', async () => {
    vi.mocked(fetch).mockResolvedValue(
      new Response(JSON.stringify({ total: 2, imported: 2, duplicateCount: 0, errorCount: 0 }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      }),
    )
    const rawText = 'http://proxy.example.com:8080\nsocks5://proxy.example.com:1080'

    await expect(dataGateway.importProxies(rawText, 'jp', 'direct')).resolves.toMatchObject({
      imported: 2,
    })
    expect(fetch).toHaveBeenCalledWith('/api/proxies/import', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ rawText, country: 'JP', group: 'direct' }),
    })
  })
})
...
describe('dataGateway split-mail registration source', () => {
  ...
  it('starts protocol registration with the same normalized resource selection', async () => {
    vi.mocked(fetch).mockResolvedValue(
      new Response(
        JSON.stringify({
          status: 'queued',
    ...
    await dataGateway.startProtocolRegistrationRun(2, ' tr ', ' Turkey-A ', 'standard')
    expect(fetch).toHaveBeenCalledWith('/api/runs/protocol-registration', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        count: 2,
