...
export const dataGateway = {
  ...
  async createEmailChange(accountId: string, targetEmailId: string): Promise<EmailChangeRun> {
    return parseResponse(await fetch('/api/email-changes', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ accountId, targetEmailId }),
    }))
  },

  async getEmailChange(runId: string): Promise<EmailChangeRun> {
    return parseResponse(await fetch(`/api/email-changes/${encodeURIComponent(runId)}`))
  },

  async cancelEmailChange(runId: string): Promise<EmailChangeRun> {
    return parseResponse(await fetch(`/api/email-changes/${encodeURIComponent(runId)}/cancel`, {
      method: 'POST',
    }))
  },
  ...
  async startProtocolRegistrationRun(
    count: number,
    country: string,
    group: string,
    emailSource: EmailSource = 'all',
    ...
    return parseResponse(
      await fetch('/api/runs/protocol-registration', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          count,
