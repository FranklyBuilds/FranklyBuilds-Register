...
def test_roxy_can_use_embedded_bridge_when_forced(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AUTOREGISTER_PROXY_BRIDGE", "always")
    create_payload: dict[str, object] = {}

    def handler(request: httpx.Request) -> httpx.Response:
        if request.url.path == "/browser/create":
            create_payload.update(json.loads(request.content.decode("utf-8")))
            return httpx.Response(200, json={"code": 0, "data": {"dirId": "dir-local-bridge"}})
        if request.url.path == "/browser/delete":
            return httpx.Response(200, json={"code": 0, "data": {}})
        return httpx.Response(200, json={"code": 0, "data": {}})
    ...
    async def scenario() -> None:
        async with RoxyClient(
            ...
            assert dir_id == "dir-local-bridge"
            await client.delete_browser(7, dir_id)
    ...
    assert proxy_info["proxyCategory"] == "SOCKS5"
    assert proxy_info["proxyUserName"] == ""
    assert proxy_info["proxyPassword"] == ""
