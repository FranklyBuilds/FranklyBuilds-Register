...
def test_checkout_approval_blocked_requests_checkout_rebuild(monkeypatch) -> None:
    ...
    with pytest.raises(cs_live_module.CheckoutApprovalBlockedError) as exc_info:
    ...
    assert approval_proxies == ["http://proxy-one.example:8080"]
    assert exc_info.value.rebuild_checkout is True
    assert exc_info.value.retryable is True
    assert exc_info.value.side_effect_started is False
...
@pytest.mark.parametrize("stage", ["payment_confirmation", "approval", "redirect_resolution", "poll"])
def test_task_manager_does_not_replay_side_effect_failure(stage: str) -> None:
    attempts = 0

    def side_effect_failure(_config: ExtractionConfig, **kwargs) -> dict:
        nonlocal attempts
        attempts += 1
        kwargs["stage_callback"](stage)
        error = ProtocolError(502, f"{stage} response lost")
        error.side_effect_started = True
        error.error_stage = stage
        raise error

    manager = TaskManager(extractor=side_effect_failure, max_workers=1)  # type: ignore[arg-type]
    config = ExtractionConfig(
        access_token="header.payload.signature",
        checkout_proxy="http://proxy-one.example:8080",
        update_proxy="http://update-one.example:8080",
    ...
    try:
        ...
        assert attempts == 1
        assert snapshot["status"] == "failed"
        assert snapshot["failure_category"] == "side_effect_unknown"
        assert snapshot["retryable"] is False
        assert "未知" in snapshot["error"]
    finally:
        manager.close()
