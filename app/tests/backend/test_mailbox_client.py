...
ICSMS_KEY = "tok_TEST_fixture_123"
ICSMS_PICKUP_URL = (
    "https://icsms.top/pickup#"
    f"email=person%40example.com&key={ICSMS_KEY}"
)
...
def test_icsms_pickup_url_uses_authenticated_latest_message_api() -> None:
    response = FakeResponse(
        json.dumps(
            {
                "email": EMAIL,
                "message": {
                    "subject": "Your temporary ChatGPT verification code",
                    "date": "2026-08-22T05:10:00Z",
                    "mailboxReceivedAt": "2026-08-22T05:12:13Z",
                    "text": (
                        "Enter this temporary verification code to continue:\n"
                        "246810"
                    ),
    ...
    snapshot_result = client.fetch_snapshot(ICSMS_PICKUP_URL, EMAIL)

    assert snapshot_result.verification_code == "246810"
    assert snapshot_result.received_at_utc == datetime(
        2026, 8, 22, 5, 12, 13, tzinfo=UTC
    )
    assert snapshot_result.received_offset == "+00:00"
    assert session.requests == [
        (
            "https://icsms.top/api/pickup/messages/latest",
            {
                "headers": {
                    "Accept": "application/json",
                    "Authorization": f"Bearer {ICSMS_KEY}",
...
def test_icsms_fixed_api_does_not_depend_on_user_url_dns_resolution() -> None:
    response = FakeResponse(
        '{"email":"person@example.com","message":null}',
        headers={"Content-Type": "application/json"},
    )

    def unexpected_resolver(_host: str, _port: int) -> list[str]:
        raise AssertionError("fixed ICSMS API must not enter generic URL validation")
...
@pytest.mark.parametrize(
    ...
    [
        (
            "text",
            "Your temporary ChatGPT verification code is 246810.",
        ),
        (
            "html",
            "<p>Your temporary ChatGPT verification code is 246810.</p>",
        ),
    ],
)
def test_icsms_extracts_inline_otp_only_from_message_body(
    body_key: str,
    body: str,
) -> None:
    response = FakeResponse(
    ...
    assert snapshot_result.verification_code == "246810"
    assert snapshot_result.service_code is None
...
@pytest.mark.parametrize(
    ...
    [
        (
            "https://icsms.top/pickup#"
            f"email=other%40example.com&key={ICSMS_KEY}"
        ),
        "https://icsms.top/pickup#email=person%40example.com&key=bad-secret",
        (
            "https://icsms.top/pickup#email=person%40example.com&"
            f"key={ICSMS_KEY}&key=tok_DUPLICATE"
        ),
        (
            "https://icsms.top/pickup?email=person%40example.com#"
            f"email=person%40example.com&key={ICSMS_KEY}"
...
def test_icsms_pickup_url_rejects_invalid_credentials_without_requesting(
    url: str,
) -> None:
    session = FakeSession([])
    client = MailboxClient(session=session, resolver=public_resolver)
    ...
    assert exc_info.value.code == "mailbox_url_invalid"
    assert exc_info.value.retryable is False
    assert ICSMS_KEY not in exc_info.value.message
    assert ICSMS_KEY not in str(exc_info.value)
    assert session.requests == []


def test_icsms_missing_latest_message_returns_empty_snapshot() -> None:
    response = FakeResponse(
    ...
    assert snapshot_result.verification_code is None
    assert snapshot_result.received_at_utc is None
    assert response.closed is True


@pytest.mark.parametrize("status_code", [401, 403])
def test_icsms_authentication_failure_is_not_retryable(status_code: int) -> None:
    response = FakeResponse(
        '{"detail":"invalid pickup token"}',
    ...
    with pytest.raises(MailboxClientError) as exc_info:
        client.fetch_snapshot(ICSMS_PICKUP_URL, EMAIL)
...
def test_icsms_transient_failure_is_retryable(status_code: int) -> None:
    response = FakeResponse(
        '{"detail":"try again"}',
        status_code=status_code,
        headers={"Content-Type": "application/json"},
...
def test_icsms_invalid_request_failure_is_not_retryable(status_code: int) -> None:
    response = FakeResponse(
    ...
    assert exc_info.value.code == "mailbox_response_invalid"
    assert exc_info.value.retryable is False
    assert exc_info.value.response_body is None
    assert response.closed is True


def test_icsms_stream_failure_is_retryable() -> None:
...
def test_clash_verge_dual_stack_fake_ip_dns_uses_public_dns_verification() -> None:
    response = FakeResponse("<html><body>No mail</body></html>")
    session = FakeSession([response])
    verified_hosts: list[tuple[str, int]] = []
    ...
    def public_dns(host: str, port: int) -> list[str]:
        verified_hosts.append((host, port))
        return ["104.21.38.6", "2606:4700:3037::6815:2606"]
