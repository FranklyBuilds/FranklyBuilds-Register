...
def test_ipbright_plan_proxy_preserves_upstream_scheme(monkeypatch) -> None:
    bridge_calls: list[tuple[str, int, str, str, str]] = []
    monkeypatch.setattr(
        "backend.plan_check_service.ensure_background_server",
        lambda: None,
    )
    ...
    def bridge_url(
        ...
        bridge_calls.append((host, port, username, password, scheme))
        return "http://127.0.0.1:18796"

    monkeypatch.setattr(
        "backend.plan_check_service.chain_bridge_proxy_url",
        bridge_url,
    )
    ...
    assert bridge_calls == [
        ("sp.ipipbright.net", 1000, "user", "secret", "socks5")
    ]
...
def test_regular_plan_proxy_preserves_socks5_scheme(monkeypatch) -> None:
    monkeypatch.setattr(
        "backend.plan_check_service.ensure_background_server",
        lambda: (_ for _ in ()).throw(AssertionError("bridge must not start")),
    )
