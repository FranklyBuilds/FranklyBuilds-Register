...
class OutlookController:
    ...
    # Web 管理台可选日志回调：callable(str) → 推入 SSE ring buffer
    _log_sink = None
    ...
    def _get_ip_info(cls, proxy_url):
        """查询代理出口 IP 国家（多源回退 + 中文名）。结果按 proxy_url 缓存。"""
        empty = {
            'country': '',
            'country_zh': '未知',
            'country_label': '未知',
        ...
        with cls._state_lock:
            cached = cls._ip_info_cache.get(proxy_url)
            if cached is not None:
                return dict(cached)
        try:
            from services.geo import lookup_ip_geo
            info = lookup_ip_geo(proxy_url, timeout=4.0)
        except Exception:
            info = dict(empty)
        # 兼容旧字段：查询失败时不再写 '??'，空字符串表示未知
        if not info.get('country'):
            info['country'] = ''
            info['country_zh'] = info.get('country_zh') or '未知'
            info['country_label'] = info.get('country_label') or '未知'
        with cls._state_lock:
    ...
    def set_task_prefix(self, task_num, total):
        ...
        cc = (info or {}).get('country') or '未知'
        zh = (info or {}).get('country_zh') or ''
        # 日志里同时带中文名，便于排查
        geo_tag = f"{zh}/{cc}" if zh and zh not in (cc, '未知') and cc != '未知' else (cc or '未知')
        self.thread_local._log_prefix = f"[{task_num}/{total}-{geo_tag}-{ip_short}]"
    ...
    def reset_shared_state(cls):
        with cls._state_lock:
            ...
            cls._proxy_config = None  # 下次按最新 config 重新解析代理
            cls._attempts = 0
            cls._success = 0
            cls._b2_attempts = {'click': 0, 'dblclick': 0, 'hold': 0}
            cls._b2_success = {'click': 0, 'dblclick': 0, 'hold': 0}
    ...
    def clean_up(self, page=None, type="all_browser"):
        ...
        if type == "done_browser":
            ...
            def _safe_close(obj, meth_name):
                try:
                    meth = getattr(obj, meth_name, None)
                    if meth is None:
                        return
                    # 单资源 close 放子线程，避免某个浏览器卡死拖死整个 stop
                    th = threading.Thread(target=meth, name=f"pw-{meth_name}", daemon=True)
                    th.start()
                    th.join(timeout=5.0)
                except Exception:
                    pass
            ...
            for browser in browsers:
                _safe_close(browser, "close")
            for playwright in playwights:
                _safe_close(playwright, "stop")
            self._cleanup_user_data_dir(profile_dir)
            if hasattr(self.thread_local, '_browser_profile_dir'):
                delattr(self.thread_local, '_browser_profile_dir')
            if hasattr(self.thread_local, '_fingerprint_seed'):
