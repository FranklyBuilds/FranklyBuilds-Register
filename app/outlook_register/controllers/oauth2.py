...
# === OAuth2 常量（可用 configure_oauth2 按 config.json 覆盖）===
...
def configure_oauth2(oauth_cfg=None):
    """从 config['oauth2'] 覆盖 client_id / redirect_url / Scopes。

    未提供或字段为空时保留模块默认值，保证 CLI 旧行为不变。
    """
    ...
    if client_id:
        CLIENT_ID = client_id
    redirect = str(cfg.get('redirect_url') or cfg.get('redirect_uri') or '').strip()
    if redirect:
        REDIRECT_URI = redirect
    scopes = cfg.get('Scopes') or cfg.get('scopes')
    if isinstance(scopes, (list, tuple)) and scopes:
        SCOPE = ' '.join(str(s).strip() for s in scopes if str(s).strip())
