...
## 快速入口（Web / Docker / Windows 一键）

| 场景 | 文档 |
| --- | --- |
| **Windows 一键安装 + 启动（含 Easy Proxies）** | 根目录 `一键安装环境.bat` / `一键启动.bat` / `一键停止.bat` · [启动说明.md](./启动说明.md) |
| **本机 Web 管理台** | [docs/BEGINNER_LOCAL_SETUP.md](./docs/BEGINNER_LOCAL_SETUP.md) |
| **无 GUI 服务器 Docker（注册机 + 代理池）** | [docs/BEGINNER_DOCKER_SETUP.md](./docs/BEGINNER_DOCKER_SETUP.md) |
| **Web 界面字段说明** | [docs/WEB_UI.md](./docs/WEB_UI.md) |

Windows 最短路径：

```text
1. 安装 Python 3.10+ 与 Node 18+
2. 双击 一键安装环境.bat
3. 双击 一键启动.bat
...
cp easy-proxies/config.example.yaml easy-proxies/config.yaml
docker compose up -d --build
# 浏览器打开 http://服务器IP:8000  （代理池管理 9091）
```
...
日志在 `log/`。CLI 下按 **Ctrl+C** 可中断：先写汇总，再关闭浏览器并清理 profile。Web 下点「停止」即可。
