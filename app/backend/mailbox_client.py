...
ICSMS_HOST = "icsms.top"
ICSMS_PICKUP_PATH = "/pickup"
ICSMS_LATEST_MESSAGE_URL = "https://icsms.top/api/pickup/messages/latest"
ICSMS_KEY_PATTERN = re.compile(r"tok_[A-Za-z0-9_-]+")
ICSMS_EMAIL_PATTERN = re.compile(r"^[^@\s]+@[^@\s]+\.[^@\s]+$")
...
LOGIN_PATTERN = re.compile(
    r"(?:chatgpt|openai).{0,80}(?:login|logs*in|signs*in).{0,80}(?:code|验证码|驗證碼)|"
    r"(?:login|logs*in|signs*in).{0,40}(?:code|验证码|驗證碼)|"
    r"登录验证码|登入驗證碼|登录代码|登入代碼",
    re.IGNORECASE | re.DOTALL,
)
EMAIL_CHANGE_PATTERN = re.compile(
    rf"(?:{VERIFICATION_PATTERN.pattern})|(?:{LOGIN_PATTERN.pattern})",
    re.IGNORECASE | re.DOTALL,
)
...
def _icsms_pickup_credentials(
    ...
    try:
        ...
        return None
    if (parsed.hostname or "").casefold() != ICSMS_HOST:
        return None
    if (
        parsed.scheme.casefold() != "https"
        or parsed.netloc.casefold() != ICSMS_HOST
        or parsed.path != ICSMS_PICKUP_PATH
        or parsed.query
    ...
    source_emails = fragment.get("email", [])
    keys = fragment.get("key", [])
    source_email = source_emails[0].strip().casefold() if len(source_emails) == 1 else ""
    key = keys[0].strip() if len(keys) == 1 else ""
    if (
        set(fragment) != {"email", "key"}
        or len(source_email) > 320
        or not ICSMS_EMAIL_PATTERN.fullmatch(source_email)
        or source_email != email.strip().casefold()
        or len(key) > 512
        or not ICSMS_KEY_PATTERN.fullmatch(key)
    ):
        raise MailboxClientError(
            "mailbox_url_invalid",
            "ICSMS 接码凭据与当前邮箱不匹配",
        )
    return source_email, key
...
def valid_icsms_pickup_url(value: str, email: str) -> bool:
    try:
        return _icsms_pickup_credentials(value, email) is not None
...
def parse_mailbox_snapshot(
    ...
    *,
    ...
    normalized_purpose = str(purpose or "verification").casefold()
    if normalized_purpose == "login":
        code_pattern = LOGIN_PATTERN
    elif normalized_purpose == "email_change":
        code_pattern = EMAIL_CHANGE_PATTERN
    else:
        code_pattern = VERIFICATION_PATTERN
    if code_pattern.search(normalized):
        ...
        if code is None:
            for index, text in enumerate(texts):
                context = "\n".join(texts[max(0, index - 12) : index + 1])
                if not code_pattern.search(context):
                    continue
                inline_code = re.search(r"(?<!\d)([0-9]{6})(?!\d)", text)
...
def _parse_laimail_snapshot(payload: str, content_type: str, *, purpose: str = "verification") -> MailboxSnapshot:
    snapshot = parse_mailbox_snapshot(payload, content_type, purpose=purpose)
...
def _parse_imap_message(payload: bytes, *, purpose: str = "verification") -> MailboxSnapshot:
    try:
    ...
    snapshot = parse_mailbox_snapshot("\n".join(texts), "text/plain", purpose=purpose)
...
class MailboxClient:
            ...
            for folder in folders:
                ...
                for message_id in reversed(message_ids[-self.max_imap_messages:]):
                    ...
                    if raw_message is None or len(raw_message) > self.max_response_bytes:
                        continue
                    snapshot = _parse_imap_message(raw_message, purpose=purpose)
    ...
    def _fetch_icsms_snapshot(
        ...
        *,
        ...
        credentials = _icsms_pickup_credentials(source, email)
        if credentials is None:
            raise MailboxClientError(
                "mailbox_url_invalid",
                "ICSMS 接码地址格式无效",
            )
        mailbox_email, key = credentials
        endpoint = ICSMS_LATEST_MESSAGE_URL
        ...
        try:
            ...
            if response.status_code == 404:
                return parse_mailbox_snapshot("{}", "application/json")
            ...
            message_payload = {
                ...
                if isinstance(message, dict) and message.get(key) is not None
            }
            snapshot = parse_mailbox_snapshot(
            ...
            received: tuple[datetime, str] | None = None
            if isinstance(message, dict):
                for timestamp_key in ("mailboxReceivedAt", "date"):
                    timestamp = message.get(timestamp_key)
                    if isinstance(timestamp, str):
                        received = parse_mail_datetime(timestamp)
            ...
            return MailboxSnapshot(
                ...
                verification_code=snapshot.verification_code,
                received_at_utc=(
                    received[0] if received is not None else snapshot.received_at_utc
                ),
                received_offset=(
                    received[1] if received is not None else snapshot.received_offset
                ),
                service_code=snapshot.service_code,
                service_success=snapshot.service_success,
                response_body=payload,
                received_precision_seconds=snapshot.received_precision_seconds,
            )
        finally:
    ...
    def fetch_snapshot(
        ...
        *,
        ...
        try:
            parsed_source = urlsplit(access_url)
        ...
        if source_scheme == MAILCOM_IMAP_SCHEME:
            return self._fetch_mailcom_snapshot(access_url, email, purpose=purpose)
        if source_host == ICSMS_HOST:
            return self._fetch_icsms_snapshot(access_url, email, purpose=purpose)
        ...
        try:
            ...
            if _is_laimail_list_url(current_url):
                list_snapshot = parse_mailbox_snapshot(payload, content_type, purpose=purpose)
                for message_url in _laimail_message_urls(
                    ...
                    try:
                        message_snapshot = self.fetch_snapshot(message_url, email, purpose=purpose)
            ...
            if (urlsplit(current_url).hostname or "").casefold() in LAIMAIL_HOSTS:
                return _parse_laimail_snapshot(payload, content_type, purpose=purpose)
            return parse_mailbox_snapshot(payload, content_type, purpose=purpose)
        finally:
            if response is not None:
                response.close()
    ...
    async def get_snapshot(
        ...
        *,
        ...
        return await asyncio.to_thread(self.fetch_snapshot, access_url, email, purpose=purpose)
    ...
    async def wait_for_new_code(
        ...
        *,
        ...
        async def get_snapshot_for_purpose(url: str) -> MailboxSnapshot:
            """Keep injected/legacy mailbox clients compatible with login OTP routing."""
            try:
                return await self.get_snapshot(url, email, purpose=purpose)
            except TypeError as exc:
                if "purpose" not in str(exc):
                    raise
                return await self.get_snapshot(url, email)

        while True:
            poll_count += 1
            request_started = monotonic_now()
            try:
                snapshot = await get_snapshot_for_purpose(poll_url)
            ...
            if (
                ...
                try:
                    content_snapshot = await get_snapshot_for_purpose(content_fallback_url)
                except MailboxClientError as exc:
                    if not exc.retryable:
                        raise
                    last_retryable_error = exc
