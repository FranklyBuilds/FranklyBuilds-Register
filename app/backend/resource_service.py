...
from .proxy_parser import ProxyParseError, parse_proxy_entry, proxy_input_tokens
...
def non_email_change_reservation_filter() -> dict[str, Any]:
    return {
        "$or": [
...
def valid_url(value: str, email: str | None = None) -> bool:
    try:
        ...
        if "#" not in value:
            return True
        return bool(email) and valid_icsms_pickup_url(value, email or "")
    except ValueError:
        return False
...
class MongoResourceStore:
    ...
    async def release_orphaned_reservations(self, active_run_ids: list[str]) -> int:
        # Email-change owns its reservation lifecycle; generic registration
    ...
    async def create_account(self, incoming: AccountCreate) -> AccountRecord:
        ...
        if claimed_email_id:
            ...
            # mailbox when one exists; this serializes the identity with the
            # email-change reservation without affecting addresses outside the
            # mailbox pool.
            matching_change = await self._guard(
            ...
            if matching_change is not None:
                raise DuplicateResourceError("邮箱正在进行换绑，请稍后再创建账号")
            claimed = await self._guard(
    ...
    async def delete_accounts(self, ids: list[str]) -> DeleteResult:
        normalized_ids = list(dict.fromkeys(str(value) for value in ids if str(value).strip()))
        if not normalized_ids:
            return DeleteResult(deleted=0)
        locked = await self._guard(
            self.accounts.find_one(
                {
                    "_id": {"$in": normalized_ids},
                    "emailChangeRunId": {"$exists": True, "$nin": [None, ""]},
                },
                {"_id": 1},
            )
        )
        if locked is not None:
            raise DuplicateResourceError("账号正在进行邮箱换绑，请先完成或取消任务")
        result = await self._guard(
    ...
    async def upsert_email(
        ...
        *,
        ...
        try:
            ...
            # A concurrent email-change reservation owns the unique mailbox
            # identity; treat the import as a duplicate instead of mutating it.
            return False
    ...
    async def complete_protocol_registration_success(
        ...
        *,
        ...
        if not token:
            raise ValueError("协议注册 Access Token 不能为空")
        if (
            access_token_expires_at.tzinfo is None
            or access_token_expires_at.utcoffset() is None
        ):
            raise ValueError("协议注册 Access Token 过期时间必须包含时区")

        completed_at = utc_now()
        expires_at = access_token_expires_at.astimezone(timezone.utc)
        if expires_at <= completed_at:
            raise ValueError("协议注册 Access Token 已过期")

        normalized_totp = ""
        if str(totp_secret or "").strip():
            try:
                normalized_totp = normalize_totp_secret(str(totp_secret))
            except ValueError as exc:
                raise ValueError("协议注册 TOTP Secret 无效") from exc
        normalized_totp_activated_at: datetime | None = None
        if totp_activated_at is not None:
            if (
                totp_activated_at.tzinfo is None
                or totp_activated_at.utcoffset() is None
            ):
                raise ValueError("协议注册 TOTP 激活时间必须包含时区")
            normalized_totp_activated_at = totp_activated_at.astimezone(timezone.utc)

        normalized_email = normalize_email(source["email"])
        document = {
            "_id": str(uuid4()),
            "email": source["email"],
            "emailNormalized": normalized_email,
        ...
        # Keep the insert-only defaults disjoint from the update modifiers.
        # MongoDB rejects an update when the same path appears in both
        # ``$setOnInsert`` and ``$set``.  A newly-created account still needs
        # the legacy empty field when enrollment was skipped or failed.
        ...
        set_values: dict[str, Any] = {
            ...
            "accessTokenUpdatedAt": completed_at,
        }
        # Keep the session lease private to Mongo. It is intentionally omitted
        # by AccountRecord projections/API responses, but lets maintenance
        # workflows reuse the exact protocol session when available.
        if str(cookie_header or "").strip():
            set_values["cookieHeader"] = str(cookie_header).strip()
        if isinstance(auth_session, dict) and auth_session:
            set_values["authSession"] = auth_session
        if str(refresh_token or "").strip():
            set_values["refreshToken"] = str(refresh_token).strip()
        if str(device_id or "").strip():
            set_values["deviceId"] = str(device_id).strip()
        # Do not erase a previously enrolled factor on an idempotent retry
        # whose TOTP step was skipped or temporarily failed.
        if normalized_totp:
            set_values.update(
                {
                    "totpSecret": normalized_totp,
                    "totpStatus": "enabled",
                    "totpError": None,
                }
            )
            if normalized_totp_activated_at is not None:
                set_values["totpActivatedAt"] = normalized_totp_activated_at
        elif str(totp_error or "").strip():
            set_values.update(
                {
                    "totpStatus": "failed",
                    "totpError": str(totp_error).strip()[:160],
                }
            )
        ...
        if stored is None:
            raise ResourceNotFoundError("协议注册账号完成后无法读取账号记录")
        return self._account_record(stored)
    ...
    async def payment_extractor_proxy_pool(self, country: str, group: str) -> list[str]:
        ...
        for item in documents:
            ...
            lines.append(f"{scheme}://{auth}{host_text}:{port}")
        return lines
        ...
        *,
        ...
        for document in documents:
            ...
            if proxy_scheme not in SUPPORTED_PROXY_SCHEMES:
                proxy_scheme = "http"
            result.append(f"{proxy_scheme}://{credentials}{host_text}:{port}")
        return result
...
class ResourceService:
    ...
    async def import_emails(self, raw_text: str) -> ImportResult:
        ...
        for raw_line in raw_text.lstrip("\ufeff").splitlines():
            ...
            is_url = valid_url(credential, email)
            is_mailcom_password = (
                bool(credential)
    ...
    async def import_proxies(
        ...
        # Keep line positions stable for the import preview.  Only remove a
        # leading BOM; trimming the whole document would shift diagnostics and
        # would also discard blank/comment lines before the first node.
        source_text = str(raw_text or "").lstrip("\ufeff")
            ...
            if isinstance(nodes, list):
                for node_index, node in enumerate(nodes, 1):
                    if not isinstance(node, dict):
                        entries.append(("", None, None, node_index))
                        continue
                ...
                entries.append(("", None, None, 1))
        else:
            for line_number, raw_line in enumerate(source_text.splitlines(), 1):
                entries.extend(
                    (token, None, None, line_number)
                    for token in proxy_input_tokens(raw_line)
                )

        for line, entry_country, entry_group, line_number in entries:
            if not line:
                total += 1
                errors += 1
                continue
            ...
            if normalized_line.lower().startswith("socks://"):
                normalized_line = "socks5://" + normalized_line.split("://", 1)[1]
            try:
                entry = parse_proxy_entry(normalized_line, line_number=line_number)
            except ProxyParseError:
                errors += 1
                continue
            scheme = entry.scheme
            # Mongo's existing unique index treats protocol as mutable
            # metadata, so keep preview/import duplicate semantics aligned
            # with that identity definition.
            key = (entry.host, entry.port, entry.username, entry.password)
            if key in seen:
                duplicates += 1
                continue
            seen.add(key)
    ...
    async def create_account(self, incoming: AccountCreate) -> AccountRecord:
        if not EMAIL_PATTERN.fullmatch(incoming.email) or not valid_url(
            incoming.emailAccessUrl,
            incoming.email,
        ):
