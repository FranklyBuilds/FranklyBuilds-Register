    ...
    def _available_proxy_filter(
        ...
        *,
        ...
        if re.fullmatch(r"[A-Z]{2}", normalized):
            ...
            if include_unknown_country:
                # Direct imports without a country are stored as ZZ.  They
                # remain valid generic exits until a health check identifies
                # their actual country, so do not make the import unusable.
                country_options.append({"country": "ZZ"})
            filters.append(
                {
                    "$or": [
                        *country_options,
