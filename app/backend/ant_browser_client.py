...
from .proxy_bridge import LocalProxyBridge, configured_bridge_mode, should_bridge_proxy
...
class AntBrowserClient:
    ...
    def __init__(
        ...
        *,
        ...
        proxy_bridge_mode: str | None = None,
    ) -> None:
        ...
        self._proxy_bridge_mode = configured_bridge_mode(
            proxy_bridge_mode if proxy_bridge_mode is not None else os.getenv("AUTOREGISTER_PROXY_BRIDGE")
        )
        self._proxy_bridges: dict[str, LocalProxyBridge] = {}
    ...
    async def aclose(self) -> None:
        ...
        bridges = list(self._proxy_bridges.values())
        self._proxy_bridges.clear()
    ...
    async def _prepare_proxy(
        self,
        proxy: ProxyLease,
    ) -> tuple[ProxyLease, LocalProxyBridge | None]:
        """Prepare the proxy URL accepted by Ant/Chromium.

        In ``auto`` mode the bridge handles authenticated HTTP(S)/SOCKS
        records, while plain SOCKS endpoints remain native.  The environment
        switch can disable or force this behavior explicitly.
        """

        if not should_bridge_proxy(
            proxy,
            browser="ant",
            mode=self._proxy_bridge_mode,
        ):
            return proxy, None

        bridge = LocalProxyBridge(upstream=proxy)
        try:
            await bridge.async_start()
        except Exception as exc:
            with suppress(Exception):
                await bridge.async_stop()
            if self._proxy_bridge_mode == "always":
                raise RoxyApiError(
                    "本机代理桥启动失败",
                    operation="proxy_bridge_start",
                    retryable=True,
                    error_kind="transport",
                ) from exc
            # Auto mode retains the native browser path when the loopback
            # listener cannot be allocated.
            return proxy, None

        return (
            replace(
                proxy,
                host=bridge.listen_host,
                port=bridge.port,
                username="",
                password="",
                scheme="socks5h",
    ...
    async def create_browser(self, _workspace_id: int, probe_id: str, proxy: ProxyLease) -> str:
        profile_proxy, bridge = await self._prepare_proxy(proxy)
        try:
            payload = await self._request(
                "POST",
                "/api/profiles",
                ...
                json_body={
                    "profile": {
                        ...
                        "proxyConfig": self._proxy_url(profile_proxy),
            ...
            if bridge is not None:
                with suppress(Exception):
                    await bridge.async_stop()
            raise
        previous = self._proxy_bridges.pop(profile_id, None)
        if previous is not None and previous is not bridge:
            with suppress(Exception):
                await previous.async_stop()
        if bridge is not None:
            self._proxy_bridges[profile_id] = bridge
        return profile_id
    ...
    async def delete_browser(self, _workspace_id: int, dir_id: str) -> None:
        bridge = self._proxy_bridges.pop(dir_id, None)
        try:
            await self._request(
                "DELETE",
