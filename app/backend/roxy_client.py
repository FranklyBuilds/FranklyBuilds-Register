...
class RoxyClient:
    ...
    async def create_browser(
        ...
        if not vendor_bridge_used and should_bridge_proxy(
            proxy,
            browser="roxy",
        ...
        try:
            ...
            if bridge is not None:
                with suppress(Exception):
                    await bridge.async_stop()
            raise
        previous = self._proxy_bridges.pop(dir_id, None)
        if previous is not None and previous is not bridge:
            with suppress(Exception):
                await previous.async_stop()
        if bridge is not None:
            self._proxy_bridges[dir_id] = bridge
        return dir_id
