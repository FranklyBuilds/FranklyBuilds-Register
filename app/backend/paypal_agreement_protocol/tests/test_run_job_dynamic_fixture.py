...
def test_run_job_dynamic_country_fixture(tmp_path, monkeypatch):
    monkeypatch.setattr(web, "METRICS_PATH", tmp_path / "protocol_metrics.json")
    monkeypatch.setattr(web, "PAYMENT_AUDIT_PATH", tmp_path / "payment_audit.jsonl")
    monkeypatch.setattr(web, "PAYMENT_AUDIT_KEY_PATH", tmp_path / ".payment_audit_hmac_key")
