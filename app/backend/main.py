...
from .email_change_adapter import build_default_email_change_adapter
...
from .installation_analysis import InstallationAnalysisError, get_installation_analysis
...
def create_app(
    ...
    email_change_service: EmailChangeService | None = None,
    email_change_adapter: Any | None = None,
) -> FastAPI:
    load_configured_env()
    ...
    configured_email_change_service = email_change_service or EmailChangeService(
        ...
        adapter=email_change_adapter
        or build_default_email_change_adapter(email_change_mailbox),
    )
    email_change_tasks: set[asyncio.Task[Any]] = set()
    email_change_startup_recovered = False

    def schedule_email_change(run_id: str) -> None:
        name = f"email-change-{run_id}"
        if any(not task.done() and task.get_name() == name for task in email_change_tasks):
            return
        task = asyncio.create_task(_run_email_change(run_id), name=name)
        email_change_tasks.add(task)
        task.add_done_callback(email_change_tasks.discard)

    async def recover_email_changes() -> None:
        nonlocal email_change_startup_recovered
        try:
            run_ids = await email_change_store.recover_active()
        except Exception:
            # Recovery is replayable and must not prevent the existing
            # registration/browser service from starting.
            return
        email_change_startup_recovered = True
        for run_id in run_ids:
            schedule_email_change(run_id)
    ...
    mongo.add_reconnect_callback(email_change_store.ensure_indexes)
    mongo.add_reconnect_callback(recover_email_changes)
    ...
    async def lifespan(_app: FastAPI):
        ...
        if mongo.online and not email_change_startup_recovered:
            await recover_email_changes()
        try:
            yield
        finally:
            ...
            for task in tuple(email_change_tasks):
                task.cancel()
            if email_change_tasks:
                await asyncio.gather(*email_change_tasks, return_exceptions=True)
            try:
                email_change_mailbox.session.close()
            except Exception:
                pass
    ...
    app = FastAPI(
        title="FranklyBuilds-Register Local Control Service",
        version="0.4.0",
        docs_url="/api/docs",
    ...
    @app.get("/api/installation-analysis")
    def installation_analysis() -> dict:
        """Expose persisted archive evidence for local diagnostics only."""
        try:
            return get_installation_analysis()
        except InstallationAnalysisError as exc:
    ...
    async def _run_email_change(run_id: str) -> None:
        try:
            await configured_email_change_service.process(run_id)
        except Exception:
            # The worker cannot know whether the remote boundary was crossed.
            # Leave the durable state untouched for reconnect recovery.
            pass

    @app.post("/api/email-changes", status_code=202, response_model=EmailChangeRun)
    async def create_email_change(payload: EmailChangeCreate) -> EmailChangeRun | dict[str, Any]:
        mongo.require_online()
        try:
            run = await email_change_store.create_run(
                payload.accountId,
                payload.targetEmailId,
            )
        except ValueError as exc:
            raise HTTPException(status_code=422, detail={"code": "email_change_invalid", "message": str(exc)}) from exc
        schedule_email_change(str(run["runId"]))
        return run

    @app.get("/api/email-changes/{run_id}", response_model=EmailChangeRun)
    async def get_email_change(run_id: str) -> EmailChangeRun | dict[str, Any]:
        mongo.require_online()
        return await email_change_store.get(run_id)

    @app.post("/api/email-changes/{run_id}/cancel", response_model=EmailChangeRun)
    async def cancel_email_change(run_id: str) -> EmailChangeRun | dict[str, Any]:
        mongo.require_online()
        return await email_change_store.request_cancel(run_id)
    ...
    @app.post("/api/runs/protocol-registration", response_model=RunState, status_code=202)
    async def start_protocol_registration_run(payload: ProtocolRegistrationRunCreate) -> RunState:
        try:
            return await run_manager.start_protocol_registration(
                payload.count,
                payload.country,
                payload.group,
                payload.emailSource,
