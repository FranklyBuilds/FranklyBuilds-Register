...
def classify_failure(exc: Exception, stage: str | None = None) -> dict[str, Any]:
    ...
    normalized_stage = str(
        stage or getattr(exc, "error_stage", "") or getattr(exc, "stage", "") or ""
    ).strip().casefold().replace("-", "_").replace(" ", "_")
    explicit_side_effect = getattr(exc, "side_effect_started", None)
    implicit_side_effect = normalized_stage in SIDE_EFFECT_STAGES and (
        network
        or "invalid json" in text
        or "response lost" in text
        or "connection" in text
    )
    side_effect_started = (
        bool(explicit_side_effect)
        if explicit_side_effect is not None
        else implicit_side_effect
    )
    rebuild_checkout = bool(getattr(exc, "rebuild_checkout", False))
    if rebuild_checkout:
    ...
    return {
        "category": category,
        # A request that reached a side-effect endpoint has an unknown outcome;
        # replaying the whole Checkout could duplicate the authorization. An
        # explicit blocked approval is different: no approval was accepted, so
        # the caller may rebuild a fresh Checkout and retry once.
        "retryable": rebuild_checkout or not side_effect_started,
        "network_error": network,
        "stage": str(stage or getattr(exc, "stage", "") or "unknown"),
        "error_kind": type(exc).__name__,
        "http_status": getattr(exc, "status_code", None),
        "side_effect_started": side_effect_started,
        "requires_reconciliation": side_effect_started and not rebuild_checkout,
        "rebuild_checkout": rebuild_checkout,
    }
        ...
        try:
            ...
            with self._lock:
                ...
                if record.retryable and record.attempt <= record.config.auto_retry_count and not record.cancel_event.is_set():
                    ...
                    record.stage = "failed"
                    record.error = (
                        "远端副作用结果未知，已停止自动重试，请先核对支付/审批状态后再处理；"
                        f"{raw_error}"
                        if record.failure_category == "side_effect_unknown"
                        else chinese_failure_reason(raw_error)
                    )
                    record.finished_at = utc_timestamp()
