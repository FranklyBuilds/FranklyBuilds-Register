...
class CheckoutApprovalBlockedError(ProtocolError):
    """Approval was rejected before acceptance; the Checkout must be rebuilt."""

    def __init__(self, detail: str = "ChatGPT checkout approval blocked; rebuild Checkout before retry"):
        super().__init__(409, detail)
        self.stage = "approval"
        self.error_stage = "approval"
        self.side_effect_started = False
        self.requires_reconciliation = False
        self.rebuild_checkout = True
        self.retryable = True


def _mark_side_effect_started(exc: Exception, stage: str) -> Exception:
    """Annotate an exception raised after a remote side-effect request began."""
    try:
        setattr(exc, "side_effect_started", True)
        setattr(exc, "error_stage", stage)
        setattr(exc, "stage", stage)
        setattr(exc, "requires_reconciliation", True)
    except Exception:
        # Third-party exception objects can be immutable; the caller still
        # receives the original error and the task stage remains available.
        pass
    return exc
...
def stripe_confirm_cs_live(
    ...
    try:
        ...
        raise _mark_side_effect_started(exc, "payment_confirmation") from exc
...
def chatgpt_approve(
    ...
    # A blocked response belongs to this Checkout identity. Replaying the
    # approval with another proxy can submit the same intent twice; the task
    # manager will retry the whole extraction and create a fresh Checkout.
    approval_proxy = proxies[0] if proxies else None
    ...
    try:
        ...
        raise _mark_side_effect_started(exc, "approval") from exc
...
def provider_redirect_after_confirm(
    ...
    if str(submission.get("state") or "") == "requires_approval":
        ...
        if stage_callback:
            stage_callback("approval", {"status": "required"})
        ...
        try:
            ...
            raise _mark_side_effect_started(exc, "poll") from exc
    try:
        return stripe_provider_poll(stripe, checkout, payment_method, poll_timeout, log, ctx)
