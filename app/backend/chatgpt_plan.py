...
def parse_accounts_check(
    ...
    *,
    ...
    # ``eligible_promo_campaigns.plus`` is broader than a free trial.  Keep
    # the historical interpretation for sparse campaign payloads (an id-only
    # campaign is trial-eligible), while recognizing explicit discount data.
    campaign_text = json.dumps(plus_campaign or {}, ensure_ascii=False).casefold()
    campaign_kind = str(
        plus_campaign.get("kind")
        or plus_campaign.get("type")
        or plus_campaign.get("promotion_kind")
        or ""
    ).casefold() if plus_campaign else ""
    discount_campaign = campaign_kind in {"discount", "half_price", "half-price"}
    discount_campaign = discount_campaign or any(
        marker in campaign_text
        for marker in ("discount", "half-price", "half_price", "50-pct", "50_pct", "percent_off", "amount_off")
    )
    free_trial_campaign = bool(plus_campaign) and not discount_campaign
