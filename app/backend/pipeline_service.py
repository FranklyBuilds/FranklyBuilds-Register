        ...
        if extraction_limit > 0:
            ...
            for item in await self._guard(cursor.to_list(length=100)):
                ...
                if retry_count >= extraction_limit:
                    continue
                extraction_error = item.get("extractionError")
                if isinstance(extraction_error, dict):
                    failure_category = str(
                        extraction_error.get("failureCategory")
                        or extraction_error.get("failure_category")
                        or ""
                    ).strip().casefold()
                    if failure_category == "side_effect_unknown" or extraction_error.get("retryable") is False:
                        # The remote payment/approval outcome is unknown. A
                        # pipeline retry would replay the same side effect;
                        # leave the item failed for reconciliation/manual review.
                        continue
                await self._guard(
                    self.items.update_one(
        ...
        for item in await self._guard(cursor.to_list(length=100)):
            ...
            if status == "succeeded":
                ...
                diagnostics = {
                    ...
                    "failureCategory": snapshot.get("failureCategory"),
