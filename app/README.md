# FranklyBuilds-Register（FB注册机）本机控制台

基准项目地址：[GPT-Register-Tool](https://github.com/2951461586/GPT-Register-Tool)。基准注册机 1
以后以该源码仓库及审阅 commit 为准；安装包归档只作历史对照，暂不作为功能依据。
...
1. 解压 `FranklyBuilds-Register-v1.0.0.zip`，保留包内目录结构。
...
4. 请求管理员权限并注册 `AutoRegisterMongoDB` Windows 服务（显示名为 `FranklyBuilds-Register MongoDB`）；
...
```text
FranklyBuilds-Register (FB注册机): Full stack
```
...
- 代理导入支持 `http://host:port`、`https://host:port`、
  `socks://host:port`（按 SOCKS5 处理）、`socks5://host:port` /
  `socks5h://host:port`、无认证 `host:port`、`user:pass@host:port`、
  `host:port:user:pass`、CSV 两字段或四字段（支持引号包裹逗号/空格），
  以及带 `proxies:` 的 YAML。启动台未选择代理分组时直接使用 Mongo
  的“默认组”，不需要 EasyProxy、Clash 或其他本机代理管理器。
- 浏览器代理桥内置在 FastAPI 进程中，不启动外部代理管理器。默认
  `AUTOREGISTER_PROXY_BRIDGE=auto` 会为 Ant/Chromium 的认证 HTTP(S)/SOCKS
  以及 `socks5h` 节点启动临时本机 SOCKS5 端口；Roxy 继续使用原生代理字段。
  如需让 Roxy 的认证/HTTP 或 `socks5h` 节点也通过本机桥转发，可设置
  `AUTOREGISTER_PROXY_BRIDGE=always`。
  profile 删除成功后桥立即关闭；设置为 `never` 可完全禁用桥并使用浏览器
  原生代理字段。
- 账号导出：`邮箱----密码----TOTP` 或 `邮箱----接码地址`。
- 启动台默认运行真实 Roxy/Playwright 多进程探测；`/api/runs/mock` 仍保留用于回归测试。
- 启动台可切换到“协议注册”：该模式复用同一邮箱来源、国家和代理池筛选，直接执行基准注册机的 HTTP 注册流程，不要求 Roxy workspace；浏览器注册仍是默认模式。
- 协议注册每个 worker 使用独立 HTTP 会话、邮箱和代理；Sentinel 默认优先运行 QuickJS SDK，失败时回退 HTTP 实现。可用 `OPENAI_SENTINEL_MODE=quickjs|http|auto` 选择策略，`auto` 为默认值。
- 最大并发任务在启动时形成快照；浏览器真实任务只要求至少存在 1 个 Roxy workspace，并固定复用 API 返回的第一个 workspace。完全没有 workspace 时不会预留邮箱或创建浏览器进程。
...
代理输入支持 HTTP、HTTPS、SOCKS5/SOCKS5H URL、无认证 `host:port`、常见四字段文本，以及
IPRocket、IPRoyal、1024Proxy 的参考项目格式。可在页面粘贴
`https://app.iprocket.io/...` 订阅地址并导入到两个代理池。普通 HTTP/SOCKS 代理无需
额外配置；上述三类特殊网关会自动使用独立的供应商兼容桥 `127.0.0.1:18796`。这个桥与
浏览器代理的内置临时桥是两条独立链路。供应商兼容桥会优先尝试可选的
...
FranklyBuilds-Register 启动命令控制，因此示例中的 `OPLL_WEB_HOST/PORT` 只作为源项目兼容说明。
